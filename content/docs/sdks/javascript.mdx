...
description: "Complete guide for the Dayopt JavaScript/TypeScript SDK. Perfect for Node.js backends and frontend applications."
...
ai:
  relatedQuestions:
    - "JavaScript SDKの使い方は？"
    - "TypeScriptでDayoptを使うには？"
    - "Node.jsでログを送信するには？"
    - "Express.jsでDayoptを使うには？"
    - "Next.jsでDayoptを設定するには？"
    - "ブラウザからログを送信できる？"
    - "ログレベルの設定方法は？"
    - "機密情報のマスキング方法は？"
  prerequisites:
...
The official Dayopt SDK for JavaScript and TypeScript applications. Works in Node.js backends and modern browsers.
...
const app = express()
const logger = new Dayopt({ /* config */ })
...
export const logger = new Dayopt({
  apiKey: process.env.BOXLOG_API_KEY!,
  service: 'nextjs-app',
