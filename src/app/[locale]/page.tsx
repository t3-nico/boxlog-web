...
export default async function Home({ params }: HomeProps) {
  ...
  // Use translations for Japanese, hardcoded for English
  const t = locale === 'jp' ? await getTranslations('pages.home') : null
  const common = locale === 'jp' ? await getTranslations('common') : null
  return (
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
        ...
        <Container className="relative">
          <div className="flex flex-col items-center justify-center min-h-screen py-24 text-center">
            ...
            >
              {locale === 'jp' ? t?.('title') : 'Build Your Next SaaS Product with Confidence'}
            </Heading>
            ...
            >
              {locale === 'jp' ? t?.('subtitle') : 'Transform your ideas into reality with our powerful platform. Get access to enterprise-grade tools, seamless integrations, and world-class support to scale your business faster than ever.'}
            </Text>
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
              <Button size="lg" className="shadow-lg hover:shadow-xl transition-shadow" asChild>
                <Link href="/signup">
                  ...
                  {locale === 'jp' ? t?.('cta') : 'Start Free Trial'}
                </Link>
              </Button>
              <Button 
                variant="outline" 
              ...
              >
                <Link href="/demo">
                  ...
                  {locale === 'jp' ? common?.('learnMore') : 'Learn More'}
                </Link>
              </Button>
            </div>
            ...
            <div className="mt-16 pt-8 border-t border-gray-200/50 dark:border-gray-700/50">
              <Text size="sm" variant="subtle" className="mb-6">
                {locale === 'jp' ? 'これらのチームから信頼されています' : 'Trusted by teams at'}
              </Text>
        ...
        <Container>
          <div className="text-center">
            <Heading as="h2" size="3xl" className="mb-4">
              {locale === 'jp' ? '始める準備はできましたか？' : 'Ready to get started?'}
            </Heading>
            <Text variant="muted" className="mb-8">
              {locale === 'jp' ? '私たちのプラットフォームですでに構築している何千ものチームに参加しましょう。' : 'Join thousands of teams already building with our platform.'}
            </Text>
            <Button asChild>
              <Link href={locale === 'jp' ? '/jp/features' : '/features'}>
                {locale === 'jp' ? '機能を探る' : 'Explore Features'}
              </Link>
            </Button>
          </div>
        </Container>
