...
const categoryConfig = {
  blog: {
    ...
    color: 'bg-[rgb(var(--info-bg))] text-[rgb(var(--info-color))]',
  ...
  releases: {
    ...
    color: 'bg-[rgb(var(--success-bg))] text-[rgb(var(--success-color))]',
    borderColor: 'border-[rgb(var(--border-secondary))]'
  },
  docs: {
    title: 'Documentation',
    ...
    color: 'bg-[rgb(var(--tag-accent-bg))] text-[rgb(var(--tag-accent-text))]',
    borderColor: 'border-[rgb(var(--border-secondary))]'
  }
}
  ...
  if (categories.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-[rgb(var(--text-tertiary))]">No categories found</p>
      </div>
    )
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      {categories.map(([category, tags]) => {
        ...
        return (
          <div key={category} className={`bg-[rgb(var(--bg-primary))] rounded-2xl border-2 ${config.borderColor} overflow-hidden`}>
            {/* Header */}
            <div className={`${config.color} px-6 py-4 border-b ${config.borderColor}`}>
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 bg-[rgb(var(--bg-primary))] rounded-lg flex items-center justify-center">
                  {config.icon}
                </div>
                <div>
              ...
              <div className="space-y-4 max-h-80 overflow-y-auto">
                {tags.slice(0, 20).map((tagData) => (
                  <Link
                    ...
                    className="flex items-center justify-between p-4 rounded-lg hover:bg-[rgb(var(--bg-secondary))] transition-colors group"
                  >
                    ...
                    <span className="text-sm text-[rgb(var(--text-tertiary))] font-medium">
                      {tagData.count}
                    </span>
                  </Link>
                ))}
